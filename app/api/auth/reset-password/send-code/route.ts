...
import { GMAIL_USER, GMAIL_PASS, APP_NAME, VERIFICATION_CODE_EXPIRE_MINUTES } from '@config_back';
...
export async function POST(request: NextRequest) {
  ...
  if (checkEmailError) {
    ...
    return new Response('Email not registered', { status: 400 });
  }
  console.log('Email already registered, contine with sending verification code');
  ...
  await transporter.sendMail({
    ...
    html: getResetPasswordEmailContent(verificationCode, VERIFICATION_CODE_EXPIRE_MINUTES),
  });
...
function getResetPasswordEmailContent(code: string, expires_in_minutes: number) {
  return `<!DOCTYPE html>
<html lang="en">
...
<body>
    <div class="container">
        ...
        <p>We received a request to reset your password for your account at ${APP_NAME}. Please use the following code
            to reset your password:</p>
        <p>Reset Code: <span class="code">${code}</span></p>
        <p>Please enter this code within ${expires_in_minutes} minutes to complete the password reset process. If you
            did not request this code, please disregard this email.</p>
        <footer>
            <p>Best regards,</p>
